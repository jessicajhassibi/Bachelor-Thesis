import os
os.environ["KMP_DUPLICATE_LIB_OK"] = "TRUE"
from .data_analysis import language_analyzer
from .data_analysis import plot_scraped_data
from .models import Group
from .wiki_scraper import scrape_and_generate_data
from .nlp import create_dataframes, create_cleaned_dataframes
from .data_helpers import get_documents_list, get_cleaned_documents_list, get_cleaned_dataframes, get_dataframes, get_groups
from .path_helpers import get_topic_modeling_path

